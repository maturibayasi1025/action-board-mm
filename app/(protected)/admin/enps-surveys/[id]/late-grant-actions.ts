...
export async function listEnpsLateSubmissionGrants(
  surveyId: string,
): Promise<EnpsLateGrantRow[]> {
  await requireOwner();
...
export type CreateEnpsLateGrantResult =
  | {
      ok: true;
      grantId: string;
      secretToken: string;
...
export async function createEnpsLateSubmissionGrant(
  surveyId: string,
  targetUserId: string,
): Promise<CreateEnpsLateGrantResult> {
  await requireOwner();
  ...
  if (insErr || !inserted) {
    console.error("createEnpsLateSubmissionGrant", insErr);
    return { ok: false, message: "付与の作成に失敗しました" };
  }

  const base = defaultUrl.replace(/\/$/, "");
  const answerUrl = `${base}/surveys/${surveyId}/late?g=${inserted.id}&t=${encodeURIComponent(secret)}`;
