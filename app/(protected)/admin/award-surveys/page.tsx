...
import { AwardH1SummaryCsvDownload } from "@/components/admin/award-h1-summary-csv-download";
import { AwardPeerReceivedCsvDownload } from "@/components/admin/award-peer-received-csv-download";
import {
          ...
          <CardHeader className="space-y-4">
            <div>
              ...
              <CardDescription className="mt-2">
                ...
                9–11月・表彰12月 / Q4:
                12–2月・表彰3月）に含まれる月次アンケートの指名を合算し、各バリューごとに票数の多い順に最大5名を表示します。期限内・期限後の回答を含みます。上位の寄せられたコメントと自己評価は、各人の行を開いて確認できます。取得件数がDB件数と一致しない場合はランキングを出さずエラーにします。月次3件の再取得合計と差分があれば警告します。発表前に下の月次内訳と、各月の「集計を見る」を突き合わせてください。
                「自己評価CSV」は選択中の四半期に含まれる月の、各メンバーのバリュー別自己評価本文を出力します。「他薦（受けた評価）CSV」は被推薦者1行で票数と推薦コメントをまとめ、詳細CSV（1推薦=1行）も同時に出力します。「上半期まとめCSV（1Q+2Q）」は選択中の会計年度の3–8月を対象に、自己評価と受けた他薦を1ファイルにまとめます。対象月はアンケートの
                year_month で絞ります。
              </CardDescription>
            </div>
            {quarterOptions.length > 0 && selected != null && (
              <div className="flex flex-wrap items-end gap-3">
                ...
                />
                <AwardPeerReceivedCsvDownload
                  year={selected.year}
                  quarter={selected.quarter}
                  disabled={ranking == null || ranking.surveyCount === 0}
                />
                <AwardH1SummaryCsvDownload year={selected.year} />
              </div>
            )}
          </CardHeader>
          <CardContent>
