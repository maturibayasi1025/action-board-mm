...
test.describe("アクションボード（Web版）のe2eテスト", () => {
  test("ログイン済み状態からトップページ確認", async ({ signedInPage }) => {
    ...
    await expect(
      signedInPage
        .locator("section")
        .getByText("テストユーザーLV.1東京都次のレベルまで40ポイント"),
  ...
  test("アカウントページ遷移が正常に動作する", async ({ signedInPage }) => {
    ...
    await expect(
      signedInPage.getByRole("button", { name: "生年月日が必要な理由" }),
    ).toBeVisible();
  ...
  test("グッジョブページ遷移 → グッジョブ完了 → グッジョブ取消が正常に動作する", async ({
    ...
    // グッジョブページの表示内容を確認
    await expect(
      signedInPage.getByText("(seed) ゴミ拾いをしよう (成果物不要)", {
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      signedInPage.getByText(
        "近所のゴミを拾ってみよう！清掃活動の報告は任意です。",
      ),
    ).toBeVisible();
    await expect(
      signedInPage.getByText("実行したら記録しよう！"),
    ).toBeVisible();
    await expect(
      signedInPage.getByRole("button", { name: "グッジョブ完了を記録する" }),
    ).toBeVisible();
    await expect(
      signedInPage.getByText(
    ...
    await expect(
      signedInPage.getByRole("heading", {
        name: "「(seed) ゴミ拾いをしよう (成果物不要)」トップ10",
      }),
    ).toBeVisible();

    // グッジョブ完了ページに遷移
    await signedInPage
      .getByRole("button", { name: "グッジョブ完了を記録する" })
      .click();
    ...
    await expect(
      signedInPage.getByText("50ポイント獲得しました！"),
    ).toBeVisible({ timeout: 10000 });
    ...
    await expect(
      signedInPage
        .locator("section")
        .getByText("テストユーザーLV.2東京都次のレベルまで45ポイント"),
    ).toBeVisible({ timeout: 10000 });
