...
宣言的なYAMLファイルでグッジョブデータを管理するシステムです。
...
- `mission_data/missions.yaml` - グッジョブ定義
- `mission_data/category_links.yaml` - カテゴリとグッジョブの紐付け
...
Cloud Buildでは、デプロイ時に自動的にグッジョブデータの同期が実行されます。

### 同期の流れ

1. Supabaseのデータベーススキーマをプッシュ
2. **グッジョブデータをYAMLファイルからデータベースに同期**
